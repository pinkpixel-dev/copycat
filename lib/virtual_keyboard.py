...
class VirtualKeyboard:
    ...
    def _detect_methods(self) -> Dict[str, bool]:
        ...
        try:
            result = subprocess.run(
                ["ydotool", "--help"], capture_output=True, timeout=5
            )
    ...
    def _type_with_xdotool(self, text: str, delay: int, human_like: bool) -> bool:
        ...
        try:
            ...
            if len(text) > chunk_size:
                return self._type_in_chunks(
                    text, chunk_size, delay, human_like, "xdotool"
                )
                ...
                for char in burst_text:
                    ...
                    if len(burst_text) > 1:  # Only add variance for multi-char bursts
                        variance = random.randint(
                            *self.typing_patterns["char_variance"]
                        )
    ...
    def _type_in_chunks(
        self, text: str, chunk_size: int, delay: int, human_like: bool, method: str
    ) -> bool:
