...
class ClipboardManager:
    ...
    def __init__(self):
        self.history_file = Path.home() / ".local/share/clipboard-helper/history.json"
    ...
    def _get_fallback(self) -> Optional[str]:
        ...
        for method in methods:
            try:
                result = subprocess.run(
                    method, capture_output=True, text=True, timeout=5
                )
                if result.returncode == 0 and result.stdout:
    ...
    def _set_fallback(self, content: str) -> bool:
        ...
        for method in methods:
            try:
                process = subprocess.Popen(method, stdin=subprocess.PIPE, text=True)
                process.communicate(input=content)
                if process.returncode == 0:
                    self._add_to_history(content)
                    return True
    ...
    def clear(self) -> bool:
        ...
        try:
            ...
            for selection in ["clipboard", "primary"]:
                subprocess.run(
                    ["xclip", "-selection", selection], input="", text=True, check=True
                )
            return True
    ...
    def get_clipboard_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "is_email": False,
            "is_code": False,
            "preview": content[:100] + ("..." if len(content or "") > 100 else "")
            if content
            else "",
        }
    ...
    def _is_code(self, content: str) -> bool:
        ...
        indicator_count = sum(
            1 for indicator in code_indicators if indicator.lower() in content_lower
        )
